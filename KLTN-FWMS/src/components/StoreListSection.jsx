...
export const StoreListSection = ({ onOpenModal, onStatusChange }) => {
  ...
  const fetchStores = useCallback(async (p = 1, searchVal = "", provinceVal = "") => {
    ...
    try {
      ...
      setError(e?.response?.data?.message ?? e.message ?? "Không thể tải cửa hàng");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleSearchChange = (e) => {
    ...
    searchTimer.current = setTimeout(() => {
      fetchStores(1, val, selectedProvince);
    }, 400);
  };
  ...
  const startItem = total === 0 ? 0 : (page - 1) * SIZES_PER_PAGE + 1;
  const endItem = Math.min(page * SIZES_PER_PAGE, total);
    ...
    <div className="lg:col-span-12 space-y-6">
        ...
        <div className="flex gap-4 w-full sm:w-auto flex-wrap sm:flex-nowrap">
          ...
          <div className="relative min-w-[230px]">
            ...
            <select
              ...
              className="w-full appearance-none pl-10 pr-10 py-3 bg-surface-container-highest border-none rounded-xl text-on-surface text-sm focus:outline-none focus:ring-2 focus:ring-primary"
            >
          ...
          <button
            onClick={() => fetchStores(page, searchTerm, selectedProvince)}
          ...
          {(searchTerm || selectedProvince) && (
            <button
              ...
              className="px-4 py-3 bg-surface-container-highest rounded-xl text-on-surface-variant hover:text-primary transition-colors text-sm font-bold"
            >
              Xóa lọc
            </button>
          )}
      ...
      {error && (
        <div className="text-sm px-4 py-3 rounded-xl bg-red-50 text-red-600 border border-red-100 flex items-center gap-2">
          ...
          {error}
          <button
            onClick={() => fetchStores(1, searchTerm, selectedProvince)}
      ...
      <div className="space-y-4">
        {loading ? (
          Array.from({ length: 4 }).map((_, i) => (
            <div
              key={i}
              className="h-20 rounded-xl animate-pulse bg-surface-container-low"
            />
          ))
        ) : stores.length === 0 ? (
        ...
        <p className="text-on-surface-variant text-sm font-medium mb-4 sm:mb-0">
          {loading ? (
            ...
            <>
              ...
              trên <span className="text-on-surface font-bold">{total}</span>{" "}
              cửa hàng
            </>
          )}
        </p>
          ...
          <nav className="flex items-center gap-2">
            ...
            {pages.map((p, idx) =>
              p === "..." ? (
                <span
                  key={`e-${idx}`}
                  className="px-2 text-on-surface-variant text-sm"
                >
                  …
                </span>
              ) : (
