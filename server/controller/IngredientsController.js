...
exports.GetIngredientTransaction=async function (req, res, next){
    try {
        const brandID = req.user.brandID;
        const getIngredientTransaction = await IngredientsRepository.getIngredientTransaction(brandID);
        return res.json(ApiSuccess.getSelect("Ingredient transaction list", getIngredientTransaction));
      } catch (error) {
        return next(error);
      }
}